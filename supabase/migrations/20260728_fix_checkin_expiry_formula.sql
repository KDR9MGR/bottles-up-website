-- Fixes a bug in the just-added expiry check (20260728_ticket_expiry_and_scan_fixes.sql):
-- some events have end_date set equal to (or before) start_date rather than null, which
-- made the naive coalesce() treat the event as over the instant it started. Always give
-- at least a 12h check-in window from start_date, and only extend it further if end_date
-- is genuinely later.
