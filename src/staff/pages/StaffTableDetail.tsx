...
// BottlesUp Server and Pay-at-Club system, section 2's "Open Table"
// (reached from My Tables). Reuses lookup_table_booking_for_checkin - the
// same staff-gated RPC the door check-in screen already uses - rather than
// building a parallel lookup. Read-only for now: Add Bottles / Request
// Payment / Record Club Payment are section 3's job, not built yet.
const StaffTableDetail = () => {
  ...
  useEffect(() => {
    if (!code) return;
    supabase
      .rpc('lookup_table_booking_for_checkin', { p_code: code })
      .then(({ data, error }) => {
        if (error || !data?.found) {
